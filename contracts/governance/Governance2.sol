...
contract Governance2 is IGovernance, AccessControlEnumerable {
    ...
    bytes32 public constant MODULE_ROLE =
    ...
    bytes32 public constant AUTHORIZATION_RATIO =
    ...
    function disableModuleForWallet(
        address wallet,
        address module,
        bool disabled
    ) public {
    ...
    {
        return
            ShardedWallet(payable(wallet)).balanceOf(user) >=
            Math.max(
                (ShardedWallet(payable(wallet)).totalSupply() *
                    getConfig(wallet, AUTHORIZATION_RATIO)) / 10**18,
                1
            );
    }
        ...
        returns (address)
    {
        ...
        return
            _globalOnlyKeys[bytes32(sig)] || local == address(0)
                ? global
                : local;
    }

    function setModule(bytes4 sig, address value) public {
        _staticcalls[msg.sender][sig] = value;
        emit ModuleSet(sig, value, msg.sender);
    }

    function setGlobalModule(bytes4 sig, address value) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
    ...
    function setConfig(bytes32 key, uint256 value) public {
        _config[msg.sender][key] = value;
        emit ConfigSet(key, value, msg.sender);
    }

    function setGlobalConfig(bytes32 key, uint256 value) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        _config[GLOBAL_CONFIG][key] = value;
        emit GlobalConfigSet(key, value);
    }

    function getGlobalOnlyKey(bytes32 key) public view returns (bool) {
        return _globalOnlyKeys[key];
    }

    function setGlobalKey(bytes32 key, bool value) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
